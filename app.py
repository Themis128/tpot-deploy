...
data_path = os.path.join(DATA_DIR, f"merged_{region}.csv") df = pd.read_csv(data_path) X, y, dates = load_and_prepare_data(df) if 'wine_type' in X.columns: X = X.drop(columns=['wine_type'])
