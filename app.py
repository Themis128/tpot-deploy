...
mask = dates.dt.year >= 2010 X = X[mask].reset_index(drop=True) y = y[mask].reset_index(drop=True) dates = dates[mask].reset_index(drop=True)

if 'wine_type' in X.columns: X = X.drop(columns=['wine_type'])
